  ...
  function parseKoshtorysGroups(rows, recognized, allBlocks) {
    ...
    for (let r = cols.headerIdx + 1; r < rows.length; r++) {
      ...
      if (typ && !name) {
        ...
        // Відкриваємо блок лише для показуваного (ненульового) підрозділу.
        if (!recognized || recognized.has(k)) {
          cur = { label: typ, items: [] };
          groups.push(cur);
        } else if (allBlocks && allBlocks.has(k)) {
          // Реальний, але НЕ показуваний блок (нульова К-сть у ПДВ/Готівка) —
          // закриваємо поточний, щоб його позиції не «прилипали» (жорстке
          // правило Анни, 2026-08-04).
          cur = null;
        }
        // Інакше — рядок із заповненим "Тип товару", але порожнім
        // "Найменування", чия назва НЕ є реальним блоком ПДВ/Готівка: це
        // ВНУТРІШНІЙ підзаголовок блоку (напр. "Провід ПВ-3", "Наконечники
        // мідно-луджені DT/JG" у "Кабельно-провідникова продукція"), а не межа
        // блоку. НЕ закриваємо поточний блок — його комплектуючі йдуть далі
        // (фікс 2026-08-15: такі підзаголовки з порожнім "Найменування"
        // передчасно закривали блок, і перелік не тягнувся).
        continue;
      }
