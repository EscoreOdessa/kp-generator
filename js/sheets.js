...
(function () {
  ...
  // recognized (Set нормалізованих назв блоків бюджету — позицій середньої
  // категорії ПДВ): якщо переданий, нову групу відкриваємо ЛИШЕ на
  // рядку-заголовку, чия назва збігається з одним із блоків. Рядки-заголовки
  // з іншими назвами — це ПІД-заголовки всередині блоку (напр. "2-полюсні
  // автомати DC 1000V" у блоці "Автоматика захисту фотоелектричних
  // модулів") — їх ігноруємо, а їхні позиції лишаються в поточному блоці
  // (інакше під-заголовок «крав» частину позицій блоку — реальний баг DC,
  // 2026-07-27).
  function parseKoshtorysGroups(rows, recognized) {
    ...
    for (let r = cols.headerIdx + 1; r < rows.length; r++) {
      ...
      if (typ && !name) {
        // Відкриваємо новий блок лише якщо назва збігається з блоком бюджету
        // (позицією ПДВ). Під-заголовки — пропускаємо, поточний блок триває.
        if (!recognized || recognized.has(blockKey(typ))) {
          cur = { label: typ, items: [] };
          groups.push(cur);
        }
        continue;
  ...
  async function loadCalcFromSheet(sheetUrlOrId, mode, opts) {
    ...
    if (opts.budgetDetail) {
      try {
        ...
        result.budgetDetail = { groups: parseKoshtorysGroups(koshtRows, recognized) };
      } catch (e) {
