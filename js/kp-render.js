...
(function () {
  ...
  function buildBudgetSections(m, b) {
    ...
    materials.forEach((it) => {
      ...
      //
      // ФОРМУЛА (запит Анни, 2026-09-04; розведено по типу розрахунку
      // 2026-09-07). G — стовпець "Ціна" Кошторису (d.price), ЗАВЖДИ трактуємо
      // як гривні. L2 — курс з вкладки "Моделювання Фін. показників роботи СЕС"
      // (b.usdRate). база($) = G / L2. Націнка блоку — те саме поле it.markup
      // (пошук за заголовком "націнка": стовпець F на вкладці ПДВ / E на
      // Готівка_ФОП), у ВІДСОТКАХ, тож застосовується як (1 + націнка).
      //
      //   • ПДВ:            Ціна($) = (база − база/6) × (1 + націнка)
      //     −база/6 знімає вхідний ПДВ 20% із закупівельної ціни.
      //   • C (Готівка_ФОП): Ціна($) = база × (1 + націнка)
      //     БЕЗ −/6: у варіанті "C" ПДВ немає, знімати нічого. Раніше спільна
      //     формула з −/6 з'їдала націнку (напр. при 30%: ×5/6×1.3 = ×1.083,
      //     тобто лишалось +8% замість +30%) — саме через це наценка "пропадала"
      //     у КП варіанта C (виправлено 2026-09-07 за запитом Анни).
      //
      // Значення зберігаємо в доларах; перерахунок у ₴ (× L2) і символ валюти
      // робить budgetCurrency() на рівні відображення. БЕЗ округлення (запит
      // Анни) — щоб Σ позицій сходилась із сумою блоку.
      if (m.detailedPrices && detail && /pv\s*кабел|автоматика\s+захисту|кабельно-?провідник|облік|заземленн/i.test(it.name)) {
        ...
        const isCash = m.clientMode === "cash"; // вариант "C" (Готівка_ФОП) — без ПДВ
        ...
        const priced = detail.map((d) => {
          ...
          const netUsd = isCash ? baseUsd : (baseUsd - baseUsd / 6); // C: без −/6; ПДВ: знімаємо ПДВ
          const unit = netUsd * (1 + mkFrac);  // × (1 + націнка блоку)
