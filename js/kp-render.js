...
(function () {
  ...
  //
  // ВИПРАВЛЕНО (2026-07-13, той самий день, друга ітерація): виявилось,
  // що зайвий рядок "Доставка до нас. Не робимо націнку!..." потрапляв у
  // бюджет тому, що findBudgetEquipItems() раніше брала ВСІ рядки категорії
  // "Обладнання" аж до наступної категорії. Анна попросила натомість
  // ЗАВЖДИ обмежувати вибір обладнання фіксованим діапазоном B3:B10 —
  // загальне правило, не прив'язане до конкретного файлу. Спершу це було
  // реалізовано через .slice(0, 8) (перші 8 елементів масиву), але цей
  // підхід виявився ненадійним — див. ВИПРАВЛЕНО (2026-07-14) нижче біля
  // findBudgetEquipItems().
  //
  // Оновлено, потім ПОВЕРНУТО НАЗАД (2026-07-13, той самий день, третя
  // ітерація): спершу кольорове розділення трьох груп (Обладнання/
  // Витратні матеріали/Роботи) було прибрано на користь товщих нижніх
  // границь (.group-last) — але Анна сказала, що це "все зіпсувало"
  // (гірше за попередній вигляд), і попросила повернути кольоровий вигляд
  // таблиці як було. Кольорові фони груп/комірок відновлено 1:1 як до
  // цієї сесії правок; лишився тільки .budget-page/.budget-layout
  // (вертикальне центрування бічної колонки приміток відносно всієї
  // сторінки) — цю частину Анна не просила відкочувати.
  ...
  // Діапазон вибору обладнання ОБМЕЖЕНО фіксовано B3:B10 (запит Анни,
  // 2026-07-13). ВИПРАВЛЕНО (2026-07-14): раніше це робилось через
  // `.slice(0, 8)` — перші 8 ЕЛЕМЕНТІВ МАСИВУ, у припущенні що вони завжди
  // 1:1 відповідають рядкам 3-10. Це припущення ламалось мовчки: будь-який
  // рядок усередині B3:B10 з порожньою назвою або нульовою кількістю/сумою
  // відфільтровується ще в parsePdvSheet (sheets.js) і взагалі не потрапляє
  // в масив items — тоді позиція "8" у масиві насправді відповідала рядку
  // 11, і зайвий рядок ("Доставка до нас...") усе одно просочувався. Тепер
  // фільтруємо за СПРАВЖНІМ номером рядка (`item.row`, див. sheets.js) —
  // працює правильно незалежно від того, скільки рядків усередині діапазону
  // виявились порожніми/нульовими. Діапазон винесено в
  // KP_CONFIG.BUDGET_EQUIP_ROW_RANGE (config.js) — те саме значення
  // використовує і js/sheets.js для розрахунку ціни групи "Обладнання".
  ...
  // (img height:100%/object-fit:contain) замість width:100%/auto-height —
  // картинка масштабується пропорційно і більша за замовчуванням; колонка
  // під картинку розширена. Оновлено ще раз того ж дня ("картинку
  // розташувати нижче і збільшити"): .shading-img вирівнює картинку до
  // низу свого блоку (align-items:flex-end замість center) і колонка під
  // неї розширена ще сильніше (див. style.css .shading-layout).
  ...
  // Оновлено 2026-07-13 (запит Анни, "все увеличить и расположить ближе к
  // центру" — перша спроба центрування не задовольнила): розмір фото,
  // імені/посади й контактів збільшено ще сильніше, і весь блок
  // .manager-body тепер додатково обмежений max-width і center по
  // горизонталі (не лише по вертикалі) — див. style.css.
