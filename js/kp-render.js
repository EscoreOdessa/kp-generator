...
(function () {
  ...
  // ЗАВЖДИ обмежувати вибір обладнання фіксованим діапазоном B3:B10 (8
  // позицій) — загальне правило, не прив'язане до конкретного файлу (див.
  // findBudgetEquipItems() нижче, .slice(0, 8)). Це вирішує проблему для
  // будь-якого майбутнього файлу-розрахунку, а не лише для одного випадку.
  //
  ...
  // Діапазон вибору обладнання ОБМЕЖЕНО фіксовано B3:B10 — 8 позицій
  // (запит Анни, 2026-07-13, повторно того ж дня): раніше бралися ВСІ
  // рядки категорії "Обладнання" аж до наступної категорії, і якщо у
  // файлі-розрахунку в ПДВ-таблиці після основного переліку обладнання
  // трапляється зайвий нетоварний рядок (напр. чиясь робоча нотатка на
  // кшталт "Доставка до нас. Не робимо націнку!..."), він теж потрапляв у
  // бюджет як "позиція обладнання". Це загальне правило для БУДЬ-ЯКОГО
  // файлу (не прив'язане до конкретного розрахунку) — категорія "1"
  // завжди починається з рядка 3 (рядок 2 — заголовок категорії), тож
  // перші 8 елементів масиву items відповідають рядкам 3-10 включно.
  // Якщо в майбутньому реальний список обладнання довший за 8 позицій,
  // потрібно буде переглянути це обмеження разом з Анною.
